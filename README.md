...
Parse the query into description, size, and max_price using regex, store it in the session.
Run search_listings with those params.
If search comes back empty, set an error message in the session and stop. It does not call the other two tools.
If there are results, grab the top one as selected_item.
Run suggest_outfit with the selected item and the wardrobe.
Run create_fit_card with the outfit suggestion and the item.
Return the session.
...
I gave Claude Code my Planning Loop and State Management sections along with my architecture diagram to build run_agent. I reviewed the output to make sure it branched on an empty search and stored values in the session dict instead of calling all three tools every time.